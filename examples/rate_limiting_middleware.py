...
class RateLimiter:
    ...
    def _cleanup_old_entries(self):
        ...
        print(
            f"[RateLimit] Cleanup complete. Tracking {len(self.requests)} keys")
        ...
        for max_requests, window_seconds in limits:
            ...
            if total_requests >= max_requests:
                ...
                if window_requests:
                    ...
                    retry_after = int(
                        window_seconds - (current_time - oldest_timestamp)) + 1
                else:
                    retry_after = int(window_seconds)

                print(
                    f"[RateLimit] Limit exceeded for {key}: {total_requests}/{max_requests} in {window_seconds}s")
...
def rate_limiting_middleware(request: Request):
    ...
    print(
        f"[RateLimit] Client IP: {client_ip}, User-Agent: {user_agent[:50]}...")
