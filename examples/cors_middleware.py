...
def cors_middleware(request: Request):
    ...
    if request.method == 'OPTIONS':
        ...
        if access_control_request_method:
            print(
                f"[CORS] Preflight for method: {access_control_request_method}")
...
def api_data(request: Request) -> Response:
    ...
    if request.method == 'GET':
        response = Response(
            '{"data": "Some important data", "timestamp": "2024-01-15T10:30:00Z"}')
    elif request.method == 'PUT':
        response = Response(
            f'{{"message": "Data updated", "body": "{request.body}"}}')
...
def api_credentials(request: Request) -> Response:
    ...
    if not auth_header:
        response = Response(
            '{"error": "Unauthorized", "message": "Authorization header required"}', status=401)
    else:
        response = Response(
            '{"message": "Authenticated data", "user": "john_doe"}')
